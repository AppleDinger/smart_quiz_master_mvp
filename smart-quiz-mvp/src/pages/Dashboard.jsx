...
export default function Dashboard({ skills, lastQuizSummary, onDownloadQuiz, onSetupMultiQuiz }) {
  ...
  if (skillEntries.length === 0 && !lastQuizSummary) return (
    <div className="bg-white p-8 rounded-xl shadow-lg text-center border-t-4 border-orange-400 mx-4">
      ...
      <p className="text-gray-600">Complete a quiz to see stats!</p>
    </div>
  );
    ...
    <div className="animate-fade-in space-y-6 mx-2 md:mx-0">
      {lastQuizSummary && (
        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-orange-500 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Last Quiz</h2>
            <p className="text-gray-600 font-medium">{lastQuizSummary.category}</p>
          </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg">
        ...
        <input className="w-full p-4 border rounded-xl mb-6 bg-gray-50 focus:ring-2 focus:ring-orange-300 outline-none" placeholder="🔍 Search skills..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="bg-orange-50 p-4 rounded-xl text-center"><div className="text-4xl font-black text-orange-500">{Math.round(avgScore * 100)}%</div><div className="text-xs font-bold text-gray-500">Mastery</div></div>
          <div className="bg-gray-100 p-4 rounded-xl text-center"><div className="text-4xl font-black text-gray-700">{totalSkills}</div><div className="text-xs font-bold text-gray-500">Skills</div></div>
        </div>
        ...
        <div className="border-t pt-6">
          <h3 className="text-xl font-bold mb-2">Multi-Topic Builder</h3>
          <div className="flex flex-wrap gap-2 mb-4 max-h-48 overflow-y-auto">
            {skillEntries.map(([n]) => (
              <button key={n} onClick={() => handleSelect(n)} className={`px-3 py-1 rounded-full text-sm border ${selectedQuizSkills[n] ? "bg-orange-500 text-white" : "bg-white text-gray-600"}`}>{n}</button>
            ))}
          </div>
          <button onClick={() => onSetupMultiQuiz(selected)} disabled={!selected.length} className="w-full bg-gray-800 text-white font-bold py-3 rounded-xl hover:bg-black disabled:opacity-50">Generate Quiz ({selected.length})</button>
        </div>
      </div>
    </div>
  );
